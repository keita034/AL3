...
void Player::Move() {
	...
	//キャラクター移動処理
	{
		...
		if (Input::GetInstance()->GetJoystickState(0, joyState)) {
			move.x += static_cast<float>(joyState.Gamepad.sThumbRX) / SHRT_MAX * kCharacterSpeed;
			move.y += static_cast<float>(joyState.Gamepad.sThumbRY) / SHRT_MAX * kCharacterSpeed;
		}
...
void Player::JoyPadReticle(const ViewProjection& viewProjection) {
	...
	//ジョイスティック状態取得
	if (Input::GetInstance()->GetJoystickState(0, joyState)) {
		spritePosition.x += static_cast<float>(joyState.Gamepad.sThumbLX) / SHRT_MAX * 5.0f;
		spritePosition.y -= static_cast<float>(joyState.Gamepad.sThumbLY) / SHRT_MAX * 5.0f;
	}
...
void Player::Initialize(
  Model* model, uint32_t textureHandle, WorldTransform* parent, const Vector3& position) {
	...
	//レティクル用テクスチャ取得
	uint32_t textureReticle = TextureManager::Load("images/reticle.png");
	//スプライト生成
